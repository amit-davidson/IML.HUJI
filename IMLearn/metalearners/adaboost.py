...
class AdaBoost(BaseEstimator):
    ...
    def __init__(self, wl: Callable[[], BaseEstimator], iterations: int):
        ...
        self.models_, self.weights_, self.D_, self.losses = [], [], [], []
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        """
        self.D_ = np.full(len(X), 1 / len(X))

        for i in range(self.iterations_):
            learner = self.wl().fit(X, y)
            error = learner.predict(X) * self.D_
            self.weights_[i] = 0.5 * np.log(1 / error - 1)
            self.models_[i] = learner

            # Calculate new weights
            self.D_ *= np.exp(-y * self.weights_[i] * learner.loss(X, y))
    ...
    def _predict(self, X):
        ...
        """
        losses: np.ndarray = np.zeros(shape=(len(self.models_)))
        for i in range(len(self.models_)):
            losses[i] = self.models_[i].loss(X)

        return np.sum(self.weights_ * losses)
